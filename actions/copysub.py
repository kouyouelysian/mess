...
about = 'goes to/creates a subfolder of a name in working folder and copies the selection of files to it. selection is set to new locations of the same files'
params = dict()
...
def execute(params, status, v=False):
	...
	for i in range(len(status.selection)):
		...
		helper.v(v, 'copying '+status.selection[i]+' --> '+join(newpath, name))
		copy2(status.selection[i], newpath)
